...
class DomApi(DomApiGeneral):
    ...
    def __init__(self, xml_content, xml_styles):
        ...
        if self.styles_dom.getElementsByTagName("style:page-master").__len__() != 0:
            self.page_master = self.styles_dom.getElementsByTagName(
                "style:page-master")[0]
        if self.styles_dom.getElementsByTagName("style:page-layout").__len__() != 0:
            self.page_master = self.styles_dom.getElementsByTagName(
                "style:page-layout")[0]
        self.document = self.content_dom.getElementsByTagName(
            "office:document-content")[0]
        ...
        for u in new_updatedict.keys():
            try:
                if new_updatedict[u].find("""%""") != -1 and u in dict:
                    ...
                    new_number = self.stringPercentToFloat(
                        new_updatedict[u]) * number
                    if unit == "pt":
    ...
    def normalizeStyleProperties(self):
        ...
        automatic_styles = self.content_dom.getElementsByTagName(
        ...
        for s in content_styles:
            ...
            if c == []:
                ...
                c = self.findChildrenByName(s, "style:properties")
            c = c[0]
            dict = self.style_properties_dict[(s.getAttribute(
                "style:name")).encode("latin-1")] or {}
            for attribute in dict.keys():
                c.setAttribute(self.openOfficeStringUtf8(
                    attribute), self.openOfficeStringUtf8(dict[attribute]))
    ...
    def transferStylesXml(self):
        ...
        outline_styles = self.styles_dom.getElementsByTagName(
            "text:outline-style")
    ...
    def getStylePropertiesDict(self, style_name):
        ...
        if self.style_dict[style_name].hasAttribute("style:parent-style-name"):
            parent = self.style_dict[style_name].getAttribute(
                "style:parent-style-name").encode("latin-1")
...
class PyOpenOffice(object):
    ...
    def oo_read(self, fname):
        ...
        for a in all:
            if a[:9] == 'Pictures/' and len(a) > 10:
                ...
                if self.save_pict:
                    f = open(os.path.join(
                        self.path, os.path.basename(a)), "wb")
                    f.write(pic_content)
                    f.close()
        z.close()
    ...
    def oo_replace(self, content):
        regex = [
            ...
            (r"<para(.*)>(.*?)<text:line-break[^>]*/>",
             "<para$1>$2</para><para$1>"),
        ]
...
if __name__ == "__main__":
    ...
    parser.add_option("-v", "--verbose", default=False,
                      dest="verbose", help="enable basic debugging")
    parser.add_option("-o", "--output", dest="output",
                      default='.', help="directory of image output")
    ...
    xsl = file(os.path.join(os.getcwd(), os.path.dirname(
        sys.argv[0]), xsl_file)).read()

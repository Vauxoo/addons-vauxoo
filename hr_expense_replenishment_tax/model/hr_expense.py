...
class hr_expense_expense(osv.Model):
    ...
    def copy(self, cr, uid, id_to_copy, default=None, context=None):
        ...
        return super(hr_expense_expense, self).copy(cr, uid, id_to_copy,
                                                    default, context=context)

    def payment_reconcile(self, cr, uid, ids, context=None):
        """
        It reconcile the expense advance and expense invoice account move lines
        """
        context = context or {}
        ...
        for exp in self.browse(cr, uid, ids, context=context):
            ...
            for voucher_brw in exp.payment_ids:
                if voucher_brw.state == 'posted':
                    ...
                    self.create_her_tax(cr, uid, exp.id,
                                        aml={}, context=context)
    ...
    def create_her_tax(self, cr, uid, ids, aml=None, context=None):
        ...
        # amount_tax_inv = sum( [invoice.amount_tax for invoice in
        #                      exp.invoice_ids] )
        #
    ...
    def _get_company_currency(self, cr, uid, exp_id, context=None):
        return self.pool.get('hr.expense.expense').browse(
            cr,
            uid,
            exp_id,
...
class account_voucher(osv.Model):
    ...
    def proforma_voucher(self, cr, uid, ids, context=None):
        ...
        cr.execute("select * from expense_pay_rel where av_id = %s",
                   (ids[0], ))
        dat = cr.dictfetchall()
        if dat:
            ...
            context.update({'payment_amount': voucher_brw.amount,
                            'date_voucher': voucher_brw.date})
            hr_expense_obj.create_her_tax(cr, uid, dat[0]['expense_id'],
                                          aml={}, context=context)
...
class account_move_line(osv.osv):
    ...
    def reconcile(self, cr, uid, ids, rec_type='auto', writeoff_acc_id=False,
        ...
        return super(account_move_line, self).reconcile(
            cr, uid, ids, type=rec_type, writeoff_acc_id=writeoff_acc_id,
            writeoff_period_id=writeoff_period_id,
            writeoff_journal_id=writeoff_journal_id,
            context=context)

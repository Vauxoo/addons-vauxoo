...
class MassMailController(http.Controller):
    ...
    def mailing(self, mailing_id, email=None, res_id=None, **post):
        ...
        if mailing.exists():
            if mailing.mailing_model == 'mail.mass_mailing.contact':
                contacts = request.env['mail.mass_mailing.contact'].sudo().search([('email', 'ilike', email)])  # noqa
    ...
    @http.route('/website_mass_mailing/is_subscriber', type='json',
                website=True, auth="public")
    def is_subscriber(self, list_id, **post):
        ...
        if email:
            contact_ids = Contacts.search(cr, SUPERUSER_ID,
                                          [('list_id', '=', int(list_id)),
                                           ('email', '=', email),
                                           ('opt_out', '=', False)],
                                          context=context)
    ...
    @http.route('/website_mass_mailing/subscribe', type='json', website=True,
                auth="public")
    def subscribe(self, list_id, email, **post):
        cr, context = request.cr, request.context
        Contacts = request.registry['mail.mass_mailing.contact']

        contact_ids = Contacts.search_read(cr, SUPERUSER_ID,
                                           [('list_id', '=', int(list_id)),
                                            ('email', '=', email)],
                                           ['opt_out'], context=context)
        if not contact_ids:
            Contacts.add_to_list(cr, SUPERUSER_ID, email, int(list_id),
                                 context=context)
        else:
            if contact_ids[0]['opt_out']:
                Contacts.write(cr, SUPERUSER_ID, [contact_ids[0]['id']],
                               {'opt_out': False}, context=context)
        # add email to session
        request.session['mass_mailing_email'] = email
        return True

    @http.route(['/website_mass_mailing/get_content'], type='json',
                website=True, auth="public")
    def get_mass_mailing_content(self, newsletter_id, **post):
        data = self.is_subscriber(newsletter_id, **post)
        mass_mailing_list = request.registry['mail.mass_mailing.list'].browse(request.cr, SUPERUSER_ID, int(newsletter_id), request.context)  # noqa
        data.update({

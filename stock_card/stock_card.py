...
class stock_card(osv.Model):
    ...
    def find_parent(self, cr, uid, ids):
        ...
        for scl in sc_line_obj.browse(cr, uid, scl_ids):
            ...
            if sp_ids:
                scl_ids = sc_line_obj.search(cr, uid, [('picking_id', '=', sp_ids[
                                             0]), ('product_id', '=', scl.product_id.id)])
    ...
    def write_aml(self, cr, uid, ids, scl_obj, q_mov, prom, acc_src, acc_dest):
        ...
        if move.picking_id:
            partner_id = move.picking_id.address_id and (
                move.picking_id.address_id.partner_id and move.picking_id.address_id.partner_id.id or False) or False
        lines = [
            (0, 0, {
             'name': move.name,
    ...
    def action_get_ready(self, cr, uid, ids, context={}):
        ...
        for id in ids:
            ...
            scp_obj.unlink(cr, uid, [
                           line.id for line in self.browse(cr, uid, id).sc_prod])
            sca_obj = self.pool.get('stock.card.account')
            sca_obj.unlink(cr, uid, [
                           line.id for line in self.browse(cr, uid, id).sc_acc])
            dict_inv = {}
            dict_cst = {}
            ...
            for prod_id in product_ids:
                ...
                if not dict_inv.get(prod_brw.product_tmpl_id.property_stock_account_input.id, False):
                    dict_inv[prod_brw.product_tmpl_id.property_stock_account_input.id] = aml_obj.search(
                        cr, uid,
                ...
                if not dict_cst.get(prod_brw.product_tmpl_id.property_account_expense.id, False):
                    dict_cst[prod_brw.product_tmpl_id.property_account_expense.id] = aml_obj.search(
                        cr, uid,
    ...
    def action_done(self, cr, uid, ids, context={}):
        ...
        for prod_id in product_ids:
            self.procesar_producto(
                cr, uid, ids, prod_id, Dict, loc_ids, inter_loc_ids, prod_loc_ids)
        ...
        self.write_new_cost(
            cr, uid, lst_scl_refac+lst_scl_refac_mov_int, loc_ids, inter_loc_ids, prod_loc_ids)
        return True
        ...
        while sml_x_pd_id:
            ...
            if not cont:
                ...
                if rpp.location_id.id == loc_ids and rpp.invoice_id.type == 'out_invoice':
                    ...
                    self.validate_venta(
                        cr, uid, ids, scl, q, total, avg, qda, no_cump, sml_x_pd_id, sml_id, seq)
                    #~ print 'seq despues operac: ',seq
                # NC COMPRA
                ...
                if rpp.location_id.id == prod_loc_ids and rpp.location_dest_id.id == loc_ids:
                    ...
                    for i in scl.in_sml_ids:
                        ...
                        if i_id in Dict[i.product_id.id]['sml']:
                            #~ print '#'*9
                            self.procesar_producto(
                                cr, uid, ids, i.product_id.id, Dict, loc_ids, inter_loc_ids, prod_loc_ids, i_id)
                            #~ print '%'*9
                            #~ time.sleep(2)
                    ...
                    if not (rpp.aml_cost_id or rpp.aml_inv_id):
                        ...
                        acc_mov_id = self.write_aml(
                            cr, uid, ids, scl, q, subtotal/q, acc_src, acc_dest)
...
#                                sc_line_obj.write(cr, uid, scl.id, valores)
            ...
            if sml_id == pending and pending not in no_cump:
                ...
                break
        Dict[prod_id].update({'sml': sml_x_pd_id, 'no_cump': no_cump,
                             'total': total, 'avg': avg, 'qda': qda, 'cont': cont})
        return True
...
#                else:
# raise osv.except_osv(_('Warning !'), _("No se encontro un periodo fiscal
# para esta fecha: '%s' por favor verificar.!") % (ret.date_ret or
# time.strftime('%Y-%m-%d')))
        ...
        for scl in self.browse(cr, uid, ids, context):
            if scl.stk_mov_id.in_sml_ids:
                scl_ids = self._get_scl_from_sm(cr, uid, [
                                                x.id for x in scl.stk_mov_id.in_sml_ids], context)

    ...
    def init(self, cr):
        ...
        #~ FROM (
            ...
            #~ WHERE account_account.active
              ...
              #~ AND account_move.state = 'posted'
              #~ --AND %s
            #~ --GROUP BY l.partner_id, l.currency_id, l.company_id, res_company.currency_id original
            #~ --GROUP BY l.partner_id, l.company_id, COALESCE( l.currency_id, res_company.currency_id)
            #~ GROUP BY l.partner_id,  COALESCE( l.currency_id, res_company.currency_id),res_company.currency_id,l.company_id
        #~ ) vw
        #~ WHERE total <> 0
        #~ '''%(move_query)

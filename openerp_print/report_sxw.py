...
class Printer(object):
    ...
    def write_print(self, file, print_report, args):
        if print_report.gs_use:
            logger.notifyChannel("info", netsvc.LOG_INFO, "GHOSTSCRIPT")
            #~ p = subprocess.Popen(args, stdout=subprocess.PIPE)
            #~ q = subprocess.Popen(["lpr", '-P', print_report.printer, " ".join(self.options)], stdin=p.stdout)
        else:
            logger.notifyChannel("info", netsvc.LOG_INFO, "LPR")
...
class report_sxw_ext(report_sxw):
    ...
    def process_header(self, cr, uid, processed_rml, context):
        if context.get('check_note_use', False):
            ...
            banco = pool.get('res.bank.entity').browse(
                cr, uid, context['bank_id'], context=None)
            self._add_header_bank(processed_rml, banco.format_h)
        elif not context.get('allow', False):
            self._add_header_bank(processed_rml, self._default_company(
                cr, uid, context).header_report)
...
def new_register_all(db):
    ...
    cr = db.cursor()
    cr.execute(
        "SELECT ir.report_name,ir.model,ir.report_rml FROM ir_act_report_xml ir, print_model_reports pm WHERE ir.id = pm.report_id")
    records = cr.dictfetchall()
    cr.close()
    for record in records:
        register_report_sxw_ext(record[
                                'report_name'], record['model'], record['report_rml'])
    return value

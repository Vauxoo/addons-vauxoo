...
class email_template_send_wizard(osv.osv_memory):
    _name = 'email_template.send.wizard.facturae'
    _inherit = 'email_template.send.wizard'
    
    def _get_template_value_custom(self, cr, uid, field, ctx):
        #print "_get_template_value_custom", uid, field, ctx
        #get_template_value_custom 1 requested {'lang': u'en_US', 'tz': False, 'active_model': u'account.invoice', 'journal_type': 'sale', 'active_ids': [7], 'type': 'out_invoice', 'active_id': 7}
        if field == "from":
            accounts = self._get_accounts(cr, uid, ctx)
            return accounts and accounts[0] or []
        if field == "attachment_ids":
            attachment_ids = self.pool.get('ir.attachment').search(cr, uid, [('res_id','=',ctx['active_id']), ('res_model','=','account.invoice')])
            return attachment_ids
        return ""
    
    def _get_accounts(self, cr, uid, context=None):
        res = super(email_template_send_wizard, self)._get_accounts(cr, uid, context=context)
        ...
        if not res:
            raise osv.except_osv(_("No se tiene cuenta o plantilla de email para FacturaE"), _("Pregunte a su administrador para configurar adecuadamente una cuenta o plantilla de email.\nTIP: Cambie el nombre de su plantilla a 'FacturaE'"))

        return res
    
    _defaults = {
        'attachment_ids': lambda self,cr,uid,ctx: self._get_template_value_custom(cr, uid, 'attachment_ids', ctx) or [],
        'from': lambda self,cr,uid,ctx: self._get_template_value_custom(cr, uid, 'from', ctx) or [],
    }
email_template_send_wizard()

'''
class email_template_send_wizard____BORRAR(osv.osv_memory):
    _name = 'email_template.send.wizard.facturae'
    #_inherit = 'email_template.send.wizard'
    ...
    def _get_accounts(self, cr, uid, context=None):
        ...
        if template.from_account:
            print "template.from_account",template.from_account
    ...
    _defaults = {
        ...
        'full_success': False,
        #'attachment_ids': lambda self,cr,uid,ctx: self._get_template_value(cr, uid, 'def_attachments', ctx),
        'attachment_ids': lambda self,cr,uid,ctx: self._get_template_value_custom(cr, uid, 'attachment_ids', ctx) or [],
        'from': lambda self,cr,uid,ctx: self._get_template_value_custom(cr, uid, 'from', ctx) or [],
    }

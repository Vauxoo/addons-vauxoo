...
class account_invoice_refund(osv.osv_memory):
    ...
    def compute_refund(self, cr, uid, ids, mode='refund', context=None):
        ...
        invo_obj.browse(cr,uid,res.get('domain',False)[1][2],context=context)[0]
        date = time.strftime('%Y/%m/%d %H:%M:%S')
        if not invo_brw.date_invoice:
            invo_obj.write(cr,uid,[invo_brw and invo_brw.id],{'date_invoice':date},context=context)
            return res
        ...
        date_2 = date and time and datetime.datetime(int(date_2[0]), int(date_2[1]), int(date_2[2]),int(date[0]),int(date[1]),int(date[2])) or False
        invo_obj.write(cr,uid,[invo_brw and invo_brw.id],{'date_invoice':date_2},context=context)
        return res

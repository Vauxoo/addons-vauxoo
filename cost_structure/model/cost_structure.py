...
class cost_structure(osv.osv):
    ...
    _columns = {
        ...
        'qty_ant': fields.float('Ant Qty', 'Qty', digits_compute=dp.get_precision('Cost Structure'), help="Quantity above, I had this product before the new calculation"),
        'ult_om': fields.many2one('product.uom', 'Last UOM', help='Measuring unit of this product obtained in the invoice from cost compute'),
        'prom_om': fields.many2one('product.uom', 'Avg UOM', help='Measuring unit average in move all'),
        'ant_om': fields.many2one('product.uom', 'Prev UOM', help='Measuring unit above of this product obtained in the invoice from cost compute'),
...
class method_price(osv.osv):
    ...
    def onchange_marginprice(self, cr, uid, ids, unit_price, margin_reference, cost_structure_id, context=None):
        ...
        if cost_structure_id:
            ...
            margin_reference and cost_brw.cost_ult and res['value'].update({'unit_price': (
                ((float(margin_reference)/100) * cost_brw.cost_ult) + cost_brw.cost_ult)})
            unit_price and cost_brw.cost_ult and res['value'].update({
                                                                     'margin_reference': ((unit_price - cost_brw.cost_ult)/cost_brw.cost_ult)*100})
        return res

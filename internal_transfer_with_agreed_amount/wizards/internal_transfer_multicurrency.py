...
class InternalTransferMulticurrency(models.TransientModel):
    ...
    def _get_currency(self):
        ...
        return payment.destination_journal_id.currency_id or payment.company_id.currency_id
    ...
    def apply(self):
        ...
        amls = self.env["account.move.line"].search([("payment_id", "=", payment.id)])
        if self.agreed_amount and self.currency_id == payment.company_id.currency_id:
            if len(amls) != 4:
                return
        ...
        elif self.agreed_amount and self.currency_id != payment.company_id.currency_id:
            aml = amls.filtered(lambda a: a.amount_currency > 0.00)
            if len(aml) > 1:
                return
            aml.move_id.button_cancel()
            aml.with_context(check_move_validity=False).write({"amount_currency": self.agreed_amount})
            aml.move_id.post()

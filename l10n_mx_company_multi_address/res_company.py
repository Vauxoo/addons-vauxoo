...
class res_company(osv.osv):
    ...
    def get_address_invoice_parent_company_id(self, cr, uid, ids, field, arg, context=None):
        ...
        for company_id in companies:
            ...
            if partner_parent:
                address_id = partner_obj.address_get(cr, uid, [partner_parent.id], ['invoice'])['invoice']
            #Validar, si tiene hijos utilizar no utilizar la main, mejor utilizar la normal company_id.partner_id.id
            elif company_id.company_address_main_id:
                address_id = company_id.company_address_main_id.id
            else:
                address_id = partner_obj.address_get(cr, uid, [company_id.partner_id.id], ['invoice'])['invoice']

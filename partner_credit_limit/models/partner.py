...
class ResPartner(models.Model):
    ...
    credit_overloaded = fields.Boolean(
        compute='_compute_credit_overloaded',
        help="Indicates when the customer has credit overloaded")
    overdue_credit = fields.Boolean(
        compute='_compute_overdue_credit', string="Late Payments", type='Boolean',
        help="Indicates when the customer has late payments")
    allowed_sale = fields.Boolean(
        compute='_compute_allowed_sale', string="Allowed Sales", type='Boolean',
    ...
    @api.multi
    def _compute_credit_overloaded(self):
        ...
        for partner in self:
            ...
            if new_currency and company.currency_id != new_currency:
                new_amount_currency = new_currency._convert(
                    new_amount, company.currency_id, company,
                    fields.Date.today())
    ...
    @api.multi
    def _compute_allowed_sale(self):
        for partner in self:

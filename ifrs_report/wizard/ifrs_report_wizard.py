...
class ifrs_report_wizard(osv.osv_memory):
    ...
    """ Wizard que permite al usuario elegir que
        periodo quiere imprimir del año fiscal """
    ...
    _columns = {
        ...
        'report_type': fields.selection(
            [('all', 'All Fiscalyear'), ('per', 'Force Period')],
        ...
        'report_format': fields.selection(
            [('pdf', 'PDF'), ('spreadsheet', 'Spreadsheet')], 'Report Format')
    }
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        if datas['report_type'] == 'all':
            datas['fiscalyear'] = wizard_ifrs.fiscalyear_id.id \
                or self._get_fiscalyear(cr, uid, context=context)
            datas['period'] = False
        else:
            datas['columns'] = 'ifrs'
        ...
        if str(wizard_ifrs.columns) == 'webkitaccount.ifrs_12' \
                and wizard_ifrs.report_format == 'spreadsheet':
            datas['columns'] = 'webkitaccount.ifrs_12_html'
        if str(wizard_ifrs.columns) == 'ifrs' \
                and wizard_ifrs.report_format == 'spreadsheet':

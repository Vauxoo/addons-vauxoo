...
class reportes_btree_report(report_sxw.rml_parse):
    ...
    def get_account_min_level(self,form):
        account_ids = form['account_ids'][0][2]
        if account_ids:
    ...
    def get_account_moves(self, form):
        ...
        if account_ids:
            ...
            where_account_ids='parent.id is not null'
        self.cr.execute("""
            SELECT subvw_final.type,subvw_final.level,subvw_final.code,subvw_final.name,subvw_final.saldo_inicial,subvw_final.debit,subvw_final.credit,
        ...
        ON subvw_level.id = subvw.id
            JOIN 
                (
                SELECT subvw.id as id_inicial, subvw.debit - subvw.credit AS saldo_inicial
                  ...
                  ORDER BY subvw_final.code
                    """,(form['date_ini'],form['date_fin'],form['nivel'],form['date_ini'],))
        ...
        res.append(amount)
        return res
report_sxw.report_sxw('report.reportes.btree.report', 'account.move', '/account_report_btree/report/account_report_btree.rml', parser=reportes_btree_report, header=False)

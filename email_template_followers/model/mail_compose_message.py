    ...
    def generate_email_for_composer(self, cr, uid, template_id,
        ...
        if values.get('partner_ids', False) and email_template.add_followers:
            ...
            partners_to_notify |= set(fo.partner_id.id for fo in
                                      partner_follower.browse(cr, SUPERUSER_ID,
                                                              fol_ids,
                                                              context=context))

            partners_followers_notify = values.get('partner_ids', []) + \
                list(partners_to_notify)
            values.update(
                {'partner_ids': list(set(partners_followers_notify))})
        return values

...
class purchase_requisition(osv.Model):
    ...
    def make_purchase_order(self, cr, uid, ids, partner_id, context=None):#method override
		...
		if res:
			requisition_user = self.browse(cr, uid, res.keys()[0], context=context).user_id
			purchase_order_obj = self.pool.get('purchase.order')
			purchase_order_obj.write(cr, uid, res[res.keys()[0]], {'user_id' : requisition_user.id})
        return res
...
class purchase_order(osv.Model):
	...
	_columns = {
		'user_id' : fields.many2one('res.users', 'Requisitor'),
	}

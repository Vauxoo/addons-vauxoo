...
class TestProductLifecycle(common.SingleTransactionCase):
    ...
    def setUp(self):
        """
        just in case need to use it
        """
        super(TestProductLifecycle, self).setUp()
        self.sellable_product = self.browse_ref('product.product_product_4c')
        self.obsolete_product = self.browse_ref('product.product_product_4b')
        self.draft_product = self.browse_ref('product.product_product_4')
        self.sellable_replacement = self.browse_ref(
            'product_lifecycle.product_product_4g')
        self.obsolete_replacement = self.browse_ref(
            'product_lifecycle.product_product_4f')

    def create_xml(self, name, model, res_id):
        """
        create a xml for the record.
        """
        ir_model = self.env['ir.model.data']
        ir_model.create({
            'name': name, 'model': model, 'res_id': res_id,
    ...
    def test_01_product_create(self):
        """
        Create a new product and check it was correctly created with the
        default product lifecycle state and the replacements products were
        link.
        """
        ...
        self.create_xml('product_product_4h', product._name, product.id)
    ...
    def test_03_product_delete(self):
        """
        Now we are going to delete the recently created xml id
        'product_lifecycle.product_product_4h' record.
        """
        product = self.browse_ref('product_lifecycle.product_product_4h')
    ...
    def test_04_product_duplicate(self):
        ...
        """
        new_product = self.obsolete_product.copy()
    ...
    def test_05_purchase_order(self):
        ...
        self.create_xml('test_purchase_order', order._name, order.id)

        # Add one sellable line (first line)
        self.create_pol(order, self.sellable_product)
        self.assertTrue(order.order_line)
        self.assertEquals(len(order.order_line), 1)
        self.assertIn(self.sellable_product,
        ...
        self.create_pol(order, self.draft_product)
        self.assertEquals(len(order.order_line), 2)
        self.assertIn(self.draft_product,
                      order.order_line.mapped('product_id'))
    ...
    def test_06_replacement_product_wizard(self):
        ...
        working propertly.
         ...
         (product_product_4c, [product_product_4g product_product_4f])
        """
        # Check if the purchase order have lines with discontiued products.
        order = self.browse_ref('product_lifecycle.test_purchase_order')
        ...
        self.assertIn(self.sellable_product,
                      order.order_line.mapped('product_id'))
        self.assertEqual(self.sellable_product.state2, 'sellable')
        self.sellable_product.state2 = 'obsolete'
        self.assertEqual(self.sellable_product.state2, 'obsolete')
        ...
        self.assertEqual(obsolete_order_line.product_id, self.sellable_product)

        # Simulate click on the "Check Discontinued Products" button to run the
        # replacemenet product wizard.
        wiz = self.env['replacement.product']
        wiz = wiz.with_context({
            'active_id': self.ref('product_lifecycle.test_purchase_order'),
            'active_ids': [self.ref('product_lifecycle.test_purchase_order')],
        ...
        self.assertEqual(wiz_line.replacement_product_id,

...
class email_template(osv.Model):
    ...
    _columns = {
        'att_default': fields.boolean(
            'Add attachment', help='Add attachment of record by default'),
        'att_other_field': fields.text(
            'Add attachment from other filed', help='specify from which fields'
            ' are to get attachments (only fields with relation to ir.'
            'attachment)')
    }
    ...
    def onchange_template_id(self, cr, uid, ids, template_id,
        ...
        if template_id:
            ...
            if template and template.att_default:
                attach = self.pool.get('ir.attachment').search(
                    cr, uid, [('res_id', '=', res_id),
                              ('res_model', '=', model)])

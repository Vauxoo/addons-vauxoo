...
class inherits_sale(osv.Model):
    ...
    def _check_unit_measure(self, cr, uid, ids, context=None):
        ...
        if sol_brw.product_id and not sol_brw.product_id.uom_id.id == sol_brw.product_uom.id:
            raise osv.except_osv(_('Error !'), _(
                "The Unit measure in the line will be the unit measure for this product %s .") % (sol_brw.product_id.name,))
        return True
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        res = super(
            inherits_sale, self).product_id_change(cr, uid, ids, pricelist, product, qty=qty,

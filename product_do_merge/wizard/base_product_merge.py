...
class MergeProductAutomatic(models.TransientModel):
    ...
    def _update_foreign_keys(self, src_products, dst_product, model=None):
        if model:
            ...
            uom_ids = self.env('product.uom').search([
                ('category_id', '=', dst_product.uom_id.category_id.id)])
    ...
    @api.multi
    def _merge(self, dst_product=None):
        proxy = self.env['product.product']
        product_ids = self.exists()
        ...
        if dst_product and dst_product in product_ids:
            src_products = self.filtered(lambda i: i != dst_product)
        else:
    ...
    def merge_cb(self, ids):
        assert is_integer_list(ids)

        this = self.with_context(active_test=False).browse(ids[0])
        ...
        if not list(product_ids):
            ...
            self._merge(product_ids, this.product_to)
        return True
    ...
    def start_process_cb(self):
        ...
        * If the user has selected the 'exclude_XXX' fields, avoid the
        ...
        this = self.with_context(active_test=False).browse(self[0])

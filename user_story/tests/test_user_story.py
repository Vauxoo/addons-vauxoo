...
class TestUserStory(TransactionCase):
    ...
    def test_create_method(self):
        ...
        self.assertRaises(
            ...
            cr, user_test_id,
            {
                ...
                'accep_crit_ids': [
                    (0, 0, {'name': 'Criterial Test 1', 'scenario': 'Test 1'}),
        ...
        # Try that a user with user story group can create a user story,
        # this group must allow create user story without problems
        ...
        }), "An user with user story group manager "
            "cannot create an user story")
    ...
    def test_unlink_method(self):
        ...
        self.assertRaises(
            except_orm, self.story.unlink, cr, user_test_id, [story_id])
        # Adding user story group to the user created previously
        self.user.write(cr, SUPERUSER_ID, [user_test_id], {
            'groups_id': [(4, us_manager_group.id)]
        })
        # Try that a user with user story group can remove a user story,
        # this group must allow create user story without problems
        self.assertTrue(
            self.story.unlink(cr, user_test_id, [story_id]),
    ...
    def test_copy_method(self):
        ...
        self.assertRaises(
            except_orm, self.story.copy, cr, user_test_id, story_id)
        # Adding user story group to the user created previously
        self.user.write(cr, SUPERUSER_ID, [user_test_id], {
            'groups_id': [(4, us_manager_group.id)]
        })
        # Try that a user with user story group can copy a user story,
        # this group must allow create user story without problems
        self.assertTrue(
            self.story.copy(cr, user_test_id, story_id),
            "An user with user story group manager cannot "
            "remove an user story")

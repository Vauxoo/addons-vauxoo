...
def user_matching(user_id):
	...
	user = read_in_source('res.users',user_id,['name','groups_id','login','password','email','signature','notification_email_send','company_id'])
	...
	if not search_in_destiny('res.users',user.get('name')):
		...
		for j in user.get('groups_id',[]):
			...
			if search_in_destiny('res.groups',group_name):
				groups_id.append(search_in_destiny('res.groups',group_name)[0])
				
		# Actualizamos el diccionario de usuario con la lista de grupos de permisos
		user.update({'groups_id': [(6,0,groups_id)]})
			
		# Ubicamos los datos de res.company en el destino, sino existe se crea
		company_id = user.get('company_id',[]) and search_in_destiny('res.company',user.get('company_id',[])[1])
		company_id = company_id and company_id[0] or create_in_destiny('res.company',{'name': user.get('company_id',[])[1]})
...
def __main__():
	...
	for user_story in user_story_dict:
		...
		# Comprobamos si tiene elementos accep_crit_ids, de ser asi creamos estos elementos en el destino
			if accep_crit_ids:
				accep_crit_dict = object_proxy.execute(DB,uid,PASS,'acceptability.criteria','read',accep_crit_ids,['name','scenario'])
				accep_crit_o2m = []
		...
		# Actualizamos el project manager en el diccionario de project del destino
		...
		# Actualizamos el diccionario de user_story
			user_story.pop('id')
			user_story.update({ "accep_crit_ids": accep_crit_ids and accep_crit_o2m, 

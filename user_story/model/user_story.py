...
class user_story(osv.Model):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if task_obj.check_access_rights(cr, uid, 'write', False):
            ...
            # with SUPERUSERID, then we will check_permision and then make
            # the sync process, if not it will pass silently.
            ...
            for ac in vals.get('accep_crit_ids'):
                if ac[2] and ac[2].get('accepted', False):
                    ...
                    body = self.body_criteria(
                        cr, uid, ids, 'template_send_email_hu',
                        criteria[1], context)
                    ...
                    subject = _(u'Acceptance criteria accepted {criteria} on '
                                'User Story {hu}'.format(
                                    criteria=criteria[1][:30],
                                    hu=hu.id))
                    self.send_mail_hu(
                        cr, uid, ids, subject, body, hu.id, users=False,
                        context=context)
    ...
    def _get_user_story_from_ptw(self, cr, uid, ids, context=None):
        result = {}
        task_ids = {}
        for work in self.pool.get('project.task.work').browse(
                cr, uid, ids, context=context):
            if work.task_id:
                result[work.task_id.id] = True
        task_ids = task_ids.keys()
        for task in self.pool.get('project.task').browse(
    ...
    _columns = {
        ...
        'owner_id': fields.many2one(
            'res.users',
            'Owner',
            help="User Story's Owner, generally the person which asked to "
            "develop this feature",
            track_visibility='always'),
        'approval_user_id': fields.many2one(
            'res.users',
            'Approver',
            help="User which approve this USer Story"),
        'code': fields.char('Code', size=64, readonly=False),
        'planned_hours': fields.float('Planned Hours'),
        'project_id': fields.many2one('project.project', 'Project',
                                      required=True),
        'description': fields.text(
            'Description', translate=True, track_visibility='onchange'),
        ...
        'user_execute_id': fields.many2one(
            ...
            "delegating work to other human resource or running it by itself. "
            "For delegate work should monitor the proper implementation of "
        ...
        'approved': fields.boolean(
            'Approved',
            help='Has been this user story approved by customer',
            track_visibility='onchange'),
    ...
    def get_body_disapproval(self, cr, uid, i, context=None):
        ...
        answers inthe do_disaproval method.
        '''
        ...
        for i in ids:
            ...
            mail_id = mail_mail.create(
                cr, uid,
                {
                    'model': 'user.story',
                    'res_id': i,
                    'subject': (u'{name} Approved the User Story with id '
                                '{number}'.format(number=i, name=user.name)),
                    'body_html': body,
                    'auto_delete': True,
                    'email_from': user.email,
                    }, context=context)
            mail_mail.send(
                cr, uid, [mail_id], recipient_ids=followers, context=context)
...
class user_story_difficulty(osv.Model):
    ...
    _columns = {
        ...
        'points': fields.integer(
            ...
            "With it you can set an order and a value in terms of effort."),
        'help': fields.text(
            'Help',
            required=True,
...
class acceptability_criteria(osv.Model):
    ...
    def _get_user_story_field(self, cr, uid, ids, fieldname,
                              arg, context=None):
        """
    ...
    _columns = {
        ...
        'difficulty_level': fields.many2one(
            ...
            'Difficulty',
            help=_('User story level estimated level, Estimated level is the '
                   'one which will be used to propose a number of hours based '
                   'on the experience of supervisors to estimate how many '
                   'hours it can take. you can set a different number of hours'
                   ' if you think the estimation is wrong')),

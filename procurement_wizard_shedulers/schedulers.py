...
class procurement_order(osv.osv):
    ...
    def run_scheduler2(self, cr, uid, automatic=False, product_ids=False, use_new_cursor=False, context=None):
        ''' Runs through scheduler.
        @param use_new_cursor: False or the dbname
        '''
        self._procure_confirm2(cr, uid, use_new_cursor=use_new_cursor, product_ids=product_ids, context=context)
        self._procure_orderpoint_confirm2(cr, uid, automatic=automatic, product_ids=product_ids,\
                use_new_cursor=use_new_cursor, context=context)

    def _procure_confirm2(self, cr, uid, ids=None, use_new_cursor=False, product_ids=False, context=None):
        '''
        ...
        try:
            ...
            while True:
                report_ids = []
                ids = procurement_obj.search(cr, uid, [('state', '=', 'confirmed'), ('procure_method', '=', 'make_to_stock'),('product_id', 'in', product_ids)], offset=offset)
    ...
    def _prepare_automatic_op_procurement2(self, cr, uid, product, warehouse, location_id, context=None):
    ...
    def create_automatic_op2(self, cr, uid, product_ids, context=None):
        """
        ...
        for warehouse in warehouse_obj.browse(cr, uid, warehouse_ids, context=context):
            ...
            for product_read in product_obj.read(cr, uid, products_ids, ['virtual_available'], context=context):
                ...
                proc_id = proc_obj.create(cr, uid,
                            self._prepare_automatic_op_procurement2(cr, uid, product, warehouse, location_id, context=context),
                            context=context)
                wf_service.trg_validate(uid, 'procurement.order', proc_id, 'button_confirm', cr)
                wf_service.trg_validate(uid, 'procurement.order', proc_id, 'button_check', cr)
        return True

    def _get_orderpoint_date_planned2(self, cr, uid, orderpoint, start_date, context=None):
        date_planned = start_date + \
                       relativedelta(days=orderpoint.product_id.seller_delay or 0.0)
        return date_planned.strftime(DEFAULT_SERVER_DATE_FORMAT)

    def _prepare_orderpoint_procurement2(self, cr, uid, orderpoint, product_qty, context=None):
        return {'name': orderpoint.name,
                'date_planned': self._get_orderpoint_date_planned2(cr, uid, orderpoint, datetime.today(), context=context),
    ...
    def _procure_orderpoint_confirm2(self, cr, uid, automatic=False, product_ids=False, \
                ...
                if prods < op.product_min_qty:
                    ...
                    if qty:
                        proc_id = procurement_obj.create(cr, uid,
                                                         self._prepare_orderpoint_procurement2(cr, uid, op, qty, context=context),
                                                         context=context)

...
class sale_order_line(osv.osv):
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        res.get('value', False) and product_brw and product_brw.uom_id and res.get(
            'value', False).update({'product_uom': product_brw.uom_id.id})
        ...
        res.get('value', False) and product_brw and product_brw.categ_id and res.get(
            'value', False).update({'categ_id': product_brw.categ_id.id})
    ...
    def price_unit(self, cr, uid, ids, price_list, product_id, qty, context=None):
        ...
        if price_list and product_id and qty:
            price_obj = self.pool.get('product.pricelist')
            price = price_obj.price_get(cr, uid, [
                                        price_list], product_id, qty, context=context)
        ...
        for order in len(ids) == 1 and self.browse(cr, uid, ids, context=context) or []:
            for line in order.order_line:
                ...
                if property_cost_structure and len(price_compute) == len([i for i in price_compute if round(line.price_unit, 2) < round(i, 2)]):
                    ...
                    product.append(
                        u'Intenta vender el producto %s a un precio mayor al estimado para su venta' % line.product_id.name)
                    res[order.id] = {'status_bool': True}

                elif not property_cost_structure:
                    product.append(
                        u'El producto %s no tiene una estructura de costo' % line.product_id.name)
                    res[order.id] = {'status_bool': True}
    ...
    def price_unit_confirm(self, cr, uid, ids, context=None):
        ...
        for line in len(ids) == 1 and sale_brw.order_line or []:
            ...
            if property_cost_structure and len(price_compute) == len([i for i in price_compute if round(line.price_unit, 2) < round(i, 2)]):
                ...
                product.append(
                    u'The product %s has not a cost structure' % line.product_id.name)

...
class TestStandardPriceUsd(TransactionCase):
    ...
    def test_02(self):
        ...
        expected_price = float_round(
            (product.standard_price_usd * 1.15) * mxn_rate, precision_rounding=self.mxn.rounding
        )
        self.assertEqual(
            float_compare(product.price, expected_price, precision_digits=2),
            0,
    ...
    def test_03(self):
        ...
        with self.assertRaisesRegexp(ValidationError, "You must have at least one supplier with price in USD"):
            self.product.write({"standard_price_usd": 880})

    def test_04(self):
        """Test constraint check_cost_and_price."""
        with self.assertRaisesRegexp(ValidationError, "You cannot create or modify a product if the cost in USD"):
    ...
    def test_sale_margin(self):
        ...
        expected_cost = float_round(product.standard_price_usd * mxn_rate, precision_rounding=self.mxn.rounding)
        margin = float_round(expected_price - expected_cost, precision_rounding=self.mxn.rounding)
        self.assertEqual(

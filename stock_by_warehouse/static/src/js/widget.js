        ...
        start: function () {
            ...
            if (info !== false) {
                _.each(info.content, function (k, v) {
                    ...
                    k.locations_quantity_formatted = formats.format["integer"](k.locations_available, "", {
                        digits: k.digits,
                    });
    ...
    var ShowPaymentLineWidget = form_common.extend({
        _render: function () {
            ...
            if (this.nodeOptions.by_location) {
                this.$el.html(
                    QWeb.render("StockAvailabilityAlert", {
                        locations_available: formats.format["integer"](
                            self.info.available_locations || 0,
                            this.field,
                            {
                ...
                this.$el.html(
                    QWeb.render("ShowWarehouseInfo", {
                        show: formats.format["float"](self.info.warehouse || "", this.field, {
                            digits: self.info.digits || "",
                        }),
                    })
                );
                ...
                if (self.info !== false) {
                    _.each(self.info.content, function (k, v) {
                        k.index = v;
                        k.available_not_res_formated = formats.format["float"](k.available_not_res, "", {
                            digits: k.digits,
                        });

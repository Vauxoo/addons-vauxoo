...
class inactive_account_wizard(osv.osv_memory):
    ...
    def _check_moves(self, cr, uid, ids, method, account=None, context=None):
        ...
        account_ids = account_obj.search(cr, SUPERUSER_ID, [('id', 'child_of', ids)])
        if line_obj.search(cr, uid, [('account_id', 'in', account_ids)]):
            if method == 'write':
                ...
                raise osv.except_osv(_('Error!'), _(
                    'You cannot remove an account that contains journal items.'))
        #Checking whether the account is set as a property to any Partner or not
        value = 'account.account,' + str(ids)
        partner_prop_acc = self.pool.get('ir.property').search(cr, uid, [(
            'value_reference','=',value)], context=context)
        conc_acc = ''
        if account:
            conc_acc = '\nAccount: ' + account
        if partner_prop_acc:

...
class account_invoice_facturae_pdf2(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(account_invoice_facturae_pdf2, self).__init__(
    ...
    def _get_company_address(self, invoice_id):
        ...
        self.partner_invoice_address = partner_obj.browse(
            self.cr, self.uid, address_id)
    ...
    def _get_facturae_data_dict(self, invoice):
        ...
        # invoice_obj = pool.get('account.invoice')
        self.taxes = [
            tax for tax in invoice.tax_line if tax.tax_percent > 0.01]
        self.taxes_ret = [
            tax for tax in invoice.tax_line if tax.tax_percent <= 0.01]
        return ""
        """
        ...
        #print "self.invoice_data_dict['Comprobante']['Impuestos']['Traslados'\
            ]",self.invoice_data_dict['Comprobante']['Impuestos']['Traslados']
        try:
            self.taxes = [ traslado['Traslado'] for traslado in self.\
                invoice_data_dict['Comprobante']['Impuestos']['Traslados'] if \
                float( traslado['Traslado']['tasa'] ) >0.01 ]
            #self.taxes = [tax for tax in invoice.account_invoice_tax if \
                tax.tax_percent > 0.01]
        ...
        #self.taxes_ret = []
        for retencion in self.invoice_data_dict['Comprobante']['Impuestos'].\
            get('Retenciones', []):
            amount_untaxed = float( self.invoice_data_dict['Comprobante'][
                'subTotal'] )
            tax_ret_amount = float( retencion['Retencion']['importe'] )
            tasa = tax_ret_amount and amount_untaxed and tax_ret_amount * \
                100 / amount_untaxed or 0.0

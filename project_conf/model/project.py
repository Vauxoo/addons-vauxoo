...
class project_task(osv.osv):
    ...
    def send_mail_task_new_test(self, cr, uid, ids, context=None):
        '''
        Send mail automatically to change task
        to Backlog and to Testing Leader.
        '''
        context = context or {}
        # Dont send context to dont get language of user in read method
        if ids.get('stage_id'):
            type_obj = self.pool.get('project.task.type').read(
                cr, uid, ids['stage_id'][0], ['name'])
            if type_obj.get('name', False) == 'Backlog':
                self.send_mail_task(
                    cr, uid, ids, 'template_send_email_task_new', context)
            elif type_obj.get('name', False) == 'Testing Leader':
                self.send_mail_task(
    ...
    _columns = {
        'project_leader_id': fields.many2one(
            ...
            """Leader state. The person should review: Work Summary, Branch """
            """and Make Functional Tests. When everything works this person """
            """should change task to done."""),
    }
    _defaults = {

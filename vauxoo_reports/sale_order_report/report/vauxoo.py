    ...
    def _get_addr(self, idpartner=None,type_r=None):
        ...
        if addr_ids: 
            ...
            if addr.country_id.code == "MX":
                lista=(addr.street and ('%s'%(addr.street)) or '') +(hasattr(addr,'l10n_mx_street4') and addr.l10n_mx_street4 and (' %s'%(addr.l10n_mx_street4)) or '')+(hasattr(addr,'street2') and addr.street2 and (', %s'%(addr.street2)) or '')+(hasattr(addr,'city') and addr.city and ('<br/> %s'%(addr.city)) or '')+(hasattr(addr,'l10n_mx_city2') and addr.l10n_mx_city2 and (', %s'%(addr.l10n_mx_city2)) or '')+(addr.state_id and (', %s'%(hasattr(addr,'state_id') and addr.state_id.name)) or '')+(hasattr(addr,'country_id') and addr.country_id and (', %s'%(addr.country_id.name)) or '')+(hasattr(addr,'zip') and addr.zip and (', %s'%(addr.zip)) or '')+(hasattr(addr,'phone') and addr.phone and ('<br/> TELF.: %s'%(addr.phone)) or '') + (hasattr(addr,'fax') and  addr.fax and (',FAX: %s'%(addr.fax)) or '') +(hasattr(addr,'mobile') and addr.mobile and (',CEL.: %s'%(addr.mobile)) or '')
            else:

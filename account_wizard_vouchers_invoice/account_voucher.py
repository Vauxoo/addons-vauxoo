...
class account_voucher(osv.Model):
    ...
    def _voucher_search(self, cr, uid, obj, name, args, context=None):
        ...
        company_id = self.pool.get('res.company').browse(cr, uid, company_user, context=context).id
        list_voucher = []
        for voucher in self.search(cr, uid, [('state','=','posted'), ('company_id', '=', company_id)]):
            reconcile = False
    ...
    _columns = {
        'conciled' : fields.function(_check_conciled, type='boolean', string='References', method=True, fnct_search=_voucher_search)
        }

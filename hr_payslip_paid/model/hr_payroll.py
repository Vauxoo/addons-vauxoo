...
class hr_payslip(osv.osv):
    ...
    def _compute_lines(self, cr, uid, ids, name, args, context=None):
        ...
        for payslip in self.browse(cr, uid, ids, context=context):
            ...
            lines = []
            if payslip.move_id:
                for pay in payslip.move_id.line_id:
                    temp_lines = []
                    if pay.reconcile_id:
                        temp_lines = \
                            [line.id for line in pay.reconcile_id.line_id]
                    elif pay.reconcile_partial_id:
                        temp_lines = \
                            [partial_line.id for partial_line in
                             pay.reconcile_partial_id.line_partial_ids]
                    lines += [x for x in temp_lines if x not in lines]
                    src.append(pay.id)

            lines = [filtered_line for filtered_line in
                     lines if filtered_line not in src]
    ...
    def _reconciled(self, cr, uid, ids, name, args, context=None):
        ...
        for pay in self.browse(cr, uid, ids, context=context):
            ...
            if not res[pay.id] and pay.state == 'paid':
                wf_service.trg_validate(uid, 'hr.payslip', pay.id,
                                        'open_test', cr)
        return res
    ...
    def _get_payslip_from_reconcile(self, cr, uid, ids, context=None):
        move = {}
        for reconcile in self.pool.get('account.move.reconcile').browse(
    ...
    _columns = {
        ...
        'reconciled': fields.function(
            ...
            store={
                ...
                'account.move.reconcile': (_get_payslip_from_reconcile,
                                           None, 50), },
    ...
    def move_line_id_payment_gets(self, cr, uid, ids, *args):
        ...
        account_ids = []
        for detail_x in payslip_bwr.details_by_salary_rule_category:
            if detail_x.salary_rule_id.account_credit.id:
                account_ids.append(detail_x.salary_rule_id.account_credit.id)
        if not account_ids:
            return res

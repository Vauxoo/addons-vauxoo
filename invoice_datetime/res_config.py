...
class account_config_settings(osv.osv_memory):
    ...
    def set_default_select_date(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            company_id = str(record.company_id and record.company_id.id or self._default_company(cr, uid))
            key_by_company_id = "acc_invoice.date_invoice_type_" + company_id
            config_parameters.set_param(cr, uid, key_by_company_id, record.select_date or '', context=context)

    def onchange_company_id(self, cr, uid, ids, company_id, context=None):
        result = super(account_config_settings, self).onchange_company_id(cr, uid, ids, company_id, context=context)
        type_date = False
        type_date = result.get('value', {})
        key_by_company_id = "acc_invoice.date_invoice_type_" + str(company_id or self._default_company(cr, uid))
        type_date['select_date'] = self.pool.get("ir.config_parameter").get_param(cr, uid, key_by_company_id, default='date', context=context)
